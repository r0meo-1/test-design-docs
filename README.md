...
Документы по бронированию и оплате — учебные примеры. Они не подтверждают
реальные инциденты, коммерческий стаж, выполненные прогоны или исправления.
Указанные стенды, суммы и результаты следует читать как условия примера.
Workflow Docs Check проверяет наличие файлов, а не работу продукта.

Отдельный [разбор подтверждённого дефекта API-проверки](examples/api-repeated-read.md)
содержит баг-репорт, тест-кейс, чек-лист и ссылки на код и CI.
Он подготовлен с помощью AI; границы доказательств указаны в документе.
...
- Примеры баг-репортов показывают структуру: шаги, ожидаемый результат и необходимые доказательства
